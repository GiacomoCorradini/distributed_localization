function plot_location(x1,y1,theta1,x2,y2,phi2,xA,yA,camera1,camera2,col_j)
    ...
    plot(x1,y1,'o')
    if ~isnan(camera1)
        plot(x_plot1,tan(theta1+camera1).*x_plot1-tan(theta1+camera1).*x1 + y1,'--',Color=col_j)
    end
    ...
    plot(x2,y2,'o')
    if ~isnan(camera2)
        plot(x_plot2,tan(theta2+camera2).*x_plot2-tan(theta2+camera2).*x2 + y2,'--',Color=col_j)
    end
    ...
    % object A
    plot(xA,yA,'.','MarkerSize',30,Color=col_j)
